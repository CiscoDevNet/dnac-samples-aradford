...
class NbClientManager(ClientManager):
    ...
    def __init__(self, server, username, password, version="v1", port=None, connect=True):
        """ Object initializer
    ...
    def __repr__(self):
        ...
        return "[DNAC: <server:{}> <username:{}> <password:{}> <version:{}>]".format(self.server,
                                                                                     self.username,
                                                                                     self.password,
                                                                                     self.version)
    ...
    def connect(self):
        ...
        self.log.info("Connecting to the DNAC north bound API client.")
        self.authenticate()

    def disconnect(self):
        """ Deletes the generated ticket and effectively disconneting the user. """

        self.log.info("Disconnecting the DNAC north bound API client.")
    ...
    def authenticate(self):
        ...
        resource_path = "/system/v1/auth/token"

        response = self.call_api("POST", resource_path, auth=HTTPBasicAuth(self.username, self.password),  raise_exception=True,
                                 response_dict=False)
        result_json = json.loads(response.text)
        if 'Token' in result_json.keys():
            ticket = result_json["Token"]
        else:
